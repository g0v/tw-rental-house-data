...
poetry run python flow.py run  # 日跑：export(1 日)→list→liststubs→seed→seedcheck→detail→deals→finalize→rawpack→parsed→parsedcheck→synthts→sync→manifest→quality→logs
