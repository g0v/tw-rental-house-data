<template lang="pug">
  main.about.w-100.mw7.center.pa4.lh-copy
    ...
    article
      h1 關於資料集
      AboutDataBrief
      vue-markdown(:toc="true", :breaks="false", :html="true" toc-id="data-set-toc").
        ## 資料內容與整理方式
        ...
        為了後續分析方便，從各網站收集的資料，經統整後，會被放入共同的表格中儲存，所有資料都使用相同的欄位儲存。
        ...
        | 所在樓層 | 網頁上標示之樓層，若為 0 ，則有可能為整棟出租，若超過建物樓高，則為自報頂加 |
        ...
        | 房數 | 目前只有整層住家提供此欄位 |
        | 客廳數 | 目前只有整層住家提供此欄位 |
        | 格局編碼 （陽台/衛浴/房/廳） | 綜合上述四項的編碼，例如四房三廳二陽台一衛浴會被標為 `_02010403` |
        | 自報頂加？ | 物件是否自己標示為頂加 |
        | 提供車位？ | - |
        ...
        | 需要停車費？ | - |
        | 月停車費 | - |
        | 每坪租金（含管理費與停車費） | 月租金、月管理費、月停車費的總和，除上物件坪數 |
        ...
        由於法律、技術與各種條件上的限制，本資料集並未收集原始物件網頁的完整紀錄。
        另外，本資料集目前絕大部分欄位都直接抓取自網站資料，沒有太多跨欄位或資料探勘的處理。
        如果你有發現有助於資料分析，但需要額外處理的資料，例如搜尋整理標題或說明欄位裡的資訊等，歡迎來信討論。
        ...
        1. 本資料集以 utf8 編碼釋出，使用微軟 Excel 可能會有無法開啟的狀況（話說 Excel 打得開 10+ 萬行的表格嘛..）。請參考[政府開放資料平台的解決方法](https://data.gov.tw/node/18765)
        2. 目前發現有些 591 的物件在已出租後，依然還是可以被搜尋到，所以雖然出租狀態可以確定是否出租，但出租天數有可能是錯的 XD 
        3. 目前尚未實做重複物件徵測，所以可能會有不同編號但卻代表同一物件的情況
