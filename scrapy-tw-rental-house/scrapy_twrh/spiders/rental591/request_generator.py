...
class RequestGenerator(RentalSpider):
    ...
    def gen_detail_request_args(self, rental_meta: DetailRequestMeta):
        ...
        #
        # Plain HTTP, like the list request. 591 renders the detail page on the
        # server, so the response already holds every field the parser reads -
        # the coordinate included, see get_coordinate_from_nuxt. Rendering it in
        # a browser used to cost a browser context, the whole JS bundle and a
        # networkidle wait per house, for a page ~10x the size.
