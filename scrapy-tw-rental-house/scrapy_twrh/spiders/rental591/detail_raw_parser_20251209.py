'''
Detail page parser for the 591 HTML which was served up to the 2026 redesign.
The date in the file name is the last day this parser was known to match 591,
see commit 6aa9b91 `feat: adapt to new 591 html` (2025-12-09).

Frozen on purpose. `HouseEtc.detail_raw` holds years of pages in this template
and `twrh-dataset/tools/rerun_detail_*.py` still re-parse them, so this file is
kept as it was instead of being adapted. Pages 591 serves today go to
detail_raw_parser_20260820, `detail_raw_parser` picks between the two.

This is also the only parser which still needs PaddleOCR: 591 used to obfuscate
price / floor / area as base64 images, and stopped doing so before 2026-08.
'''
import re
import logging
...
def parse_obfuscate_fields(response):
    ...
    for selector in img_selectors:
        ...
        # imported here rather than on top: importing ocr_utils loads the
        # PaddleOCR models, which costs seconds and hundreds of MB, and no
        # page has carried an obfuscated field since 2026
        from . import ocr_utils

        # output_path = os.path.join(os.getcwd(), 'ocr-test', selector['name'], house_id)
        # convert_base64_to_img(img, output_path)

        ret[selector['name']] = getattr(ocr_utils, selector['fn'])(img)
