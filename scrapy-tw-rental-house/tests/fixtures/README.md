...
new date and add the parser which reads it (see
`scrapy_twrh/spiders/rental591/detail_raw_parser.py`), leaving these in place.
...
- an obfuscated price / floor / area (`<wc-obfuscate-c-*>` plus an image),
  which 591 stopped serving before 2026-08 - `detail_raw_parser_20251209` is
  the parser that reads those
