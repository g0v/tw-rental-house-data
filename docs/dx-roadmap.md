...
- **list-driven 成交偵測（省爬取成本的正解，2026-08-30 提出）**：成交狀態（N/D）
  ...
  **前置驗證完成（2026-09-01，單日交叉表＋單城重掃實驗）**：單日缺席的誤殺率
  極高，且缺席主要是**爬蟲端 list 掃描不完整**（推薦位灌水使實際頁數超過宣稱
  `total_page`，尾頁最舊物件被擠出範圍；城市越大越嚴重）而非 591 端狀態——
  慢速重掃後絕大多數缺席者現身。結論：(a) 第一步是修 list 完整度——翻頁到
  空頁標記為止、不信 total_page，並把「list 捕獲數/open 數」做成每日觀測指標；
  (b) 完整度修好後重測誤殺率；(c) 終局改為「缺席觸發定向 detail 複查」而非
  直接改狀態——狀態變更仍由 detail 確認，全量 detail 降為只爬缺席者＋新物件。
