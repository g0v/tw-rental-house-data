...
| S3a export 切 parquet（讀取端） | #11 回填完（9/13）、export 讀 snapshot 落碼 | 區間 export 兩路 CSV 逐 byte 比對 3 次；manifest／quality 改讀分區。只改讀取端，可與上面重疊 | **9/17 晚讀取端上線**（`515b5df4`＋`6b5eb377`）：`export --source snapshot`、`exportcheck` stage（advisory，接在 snapshotcheck 之後）、`tools/compare_export.py`。三次 AGREE 走 9/18–9/20 早。**切換時要一併把 export stage 從 RUN_STAGES 最前面移到 snapshot 之後**——10/1 出九月包要的是 9/30 的 **final** snapshot，而它是在 10/1 那場 flow 的 snapshot stage 才摺出來的（此前只有 9/30 provisional，缺 9/30 白天各輪 sweep）。「export 排最前」原本的理由（此刻 DB 還沒被當日爬取動過）在 snapshot 路徑下自動消失：日期是顯式的 |
