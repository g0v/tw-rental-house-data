...
| 2026-09-12 08:00 | **arch-4b-closure 併入（`fc7bd912`）；關閉／成交當天保留最後已知狀態** | 維護者拍板：一戶關閉或成交那天，snapshot 與 DB 兩路都保留租金／座標等最後已知值（走「改 DB 路徑」，不在比對開特例；順便修掉公開 CSV 成交當天那列沒租金的老問題）。三件：(1) pipeline 對 detail 404／拒解析寫只帶 deal_status 的 parsed 列（`contracts.is_closure`）；(2) fold 對這種列只改 deal_status（DEAL sticky）、不清值、source／last_detail_at 不動；(3) synthts 第二段把當日 NOT_FOUND／DEAL 列的 NULL 欄從 House 補齊（只補既有列、狀態三欄不動；`--closed-only` 供回補過去日：第一段會對現在 OPENED 的戶生出當時不存在的列）。91 例。**(d) 收回一半**：stub 對照 list 解析器實際產出只漏 facilities／imgs／deal_status，那 2,450 筆稀疏列更可能是 bootstrap 照抄 9/11 的稀疏關閉／成交列、這些戶 9/12 回列後 fold 從稀疏列起算（待驗）。回補：run-cloud `synthts --closed-only`（9/11）→ `snapshotfold --bootstrap --date 2026-09-11`，今晚 02:10 自然重摺 9/12 final；9/1–9/10 的 DB 關閉列是否也補（202609 zip 一致性）待拍板 | master `fc7bd912` |
