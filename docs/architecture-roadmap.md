...
| S3a export 切 parquet（讀取端） | #11 回填完（9/13）、export 讀 snapshot 落碼 | 區間 export 兩路 CSV 逐 byte 比對 3 次；manifest／quality 改讀分區。只改讀取端，可與上面重疊 | **9/17 晚開工**（維護者 9/17 拍板；export.py 當時還沒有任何 snapshot／parquet 路徑），比對 9/19–21 |
