...
| S2b DB 歷史歸檔 S3 | rds-export.tf apply（IAM role＋KMS） | `rds-export.sh <snapshot>` 把 house_etc／house 匯成 `archive/rds/<export-id>/` Parquet（2018–2023 的 detail_dict 只在這裡）；photoids 改讀它；S5 之後不必再留 RDS snapshot | 9/14–15（匯出 1–2 h） |
| S2 house_etc 退役 | S2a 上線一晚無紅＋回補完＋S2b 匯出完成＋S1 上線一晚無紅 | 停寫 detail_dict／list_dict；rerun 只出 parquet；drop `house_etc` | 9/18 夜 |
| S3c 全戶最新狀態總表 | S3b 之後 | 月度物化：跨日 snapshot union、每戶取最後一列（取代 House 表的「全歷史現值」用途） | 10 月 |
