...
3. **觸發方式**：✅ 拍板（2026-09-03）——**月底自動接跑**，publish 包進
   container（隨 publisher 雲化落地：publisher image 補 clickhouse/git、
   deploy key 進 SSM，月底 export 完自動接 publish）。綠燈月全自動出貨；
   紅燈月照舊停在敘事關卡、人工補完後 `--resume`（屆時改雲上觸發）。
   過渡期（publisher 雲化落地前）本機維持現行人工／timer 觸發。
...
5. **JSON 格式 zip 是否同步上傳**：✅ 拍板（2026-09-03）——**不上傳**。
   JSON 版現已無人使用，出貨只上 CSV 系列；`datas/` 內的 JSON zip 產出
   去留可另議（不影響出貨流程）。
