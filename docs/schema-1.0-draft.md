...
| 項目 | 內容 |
|---|---|
| 提供家具_* | 2026-03 起至 1.0 生效前的 Raw 月包，約八成物件的家具欄因 list 頁覆寫而全為 F；Deduplicated 亦受影響。1.0 起由 detail 頁決定，list 頁不再寫入。**2026-09-18 補實測**：S3a 改讀 snapshot 分區後這個修復已可量測——同一個三日窗，DB 路徑有 3,739 戶家具欄是 `-`／失真，snapshot 路徑帶的是最後一次 detail 的值（`exportcheck` 每次會印「幾戶兩軌不同」）。也就是 1.0 的家具欄不是「重新開始收」，而是**把既有的 detail 值取回來**。 |
| 提供家具_桌子／椅子 | 2026 年起 591 合併為「桌椅」，兩欄失真，1.0 改出「桌椅」。 |
| 每坪租金（含管理費與停車費） | **1.0 起值會變、變得符合欄名**（2026-09-20 維護者確認 best effort）。0.x 的 DB 路徑：list 頁每天寫「月租／坪數」（不含費用），下次 detail 才蓋回含管理費／停車費的值，同一戶在兩公式間輪流，公開 CSV 多數日子是不含費用的那個。1.0 的 snapshot 路徑每個 list 日以新月租＋最後一次 detail 的管理費／停車費重算，沒抓過的費用當 0。exportcheck 三日窗量到 29,644 戶不同（`compare_export` 列為修正欄、印筆數）。 |
| 格局編碼（陽台/衛浴/房/廳） | **1.0 起值會變**（2026-09-20）。list 頁只給房／廳，list 解析器把陽台／衛浴填 0 組碼，0.x 的 DB 每個 list 日被蓋成 `0000` 開頭（整層住家幾乎全部）；1.0 的 snapshot 以最後一次 detail 的陽台／衛浴＋list 的房／廳重組。例：3 房 2 廳 2 衛 2 陽台，0.x 多數日子出 `_00000302`，1.0 出 `_02020302`。 |
| 附近有_*／附近的_*站數 | 2021 年 591 改版後即無資料，一直為 `-`；1.0 標 deprecated。 |
| 物件網址 | 待查 2026 月包是否為空；1.0 起由平台網址樣板生成，保證有值。 |
| 物件首次發現時間／物件最後更新時間／刊登者編碼 | **2026-09-17 拍板的三欄改對映**（S3a 之後資料來源從資料庫換成每日 snapshot 分區）：首次發現時間＝我們第一次在列表看到它；最後更新時間＝最後一次觀測（列表／詳情／當日爬取時間三者取最新）；刊登者編碼改為刊登者識別字串的雜湊（原為內部 UUID），**與 0.3 以前的編碼不可比**——跨版比對刊登者請以 1.0 起的值為一組。 |
...
3. 版本資訊放哪（拍板：方案 A）：zip 內附 `_meta.json`，記 schema 版本、產生時間、parser 版本、列數、欄位狀態、已知問題；CSV 不加「資料集版本」常數欄（版本是檔案層級屬性，不必每列重複）；JSON 匯出在頂層加 `dataset_version`。
4. 標題與說明全文正式收回，不再預告。
...
- 2026-09-20 §3.5 加「每坪租金」「格局編碼」兩列（1.0 值會變的兩欄，exportcheck 列修正欄）。
