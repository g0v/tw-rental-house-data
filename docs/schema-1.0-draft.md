...
| 項目 | 內容 |
|---|---|
| 提供家具_* | 2026-03 起至 1.0 生效前的 Raw 月包，約八成物件的家具欄因 list 頁覆寫而全為 F；Deduplicated 亦受影響。1.0 起由 detail 頁決定，list 頁不再寫入。**2026-09-18 補實測**：S3a 改讀 snapshot 分區後這個修復已可量測——同一個三日窗，DB 路徑有 3,769 戶家具欄是 `-`／失真，snapshot 路徑帶的是最後一次 detail 的值（`exportcheck` 每次會印「幾戶兩軌不同」）。也就是 1.0 的家具欄不是「重新開始收」，而是**把既有的 detail 值取回來**。 |
