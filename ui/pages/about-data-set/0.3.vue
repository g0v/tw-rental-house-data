  ...
  main.about.w-100.mw7.center.pa4.lh-copy
    ...
    article
      ...
      vue-markdown(:breaks="false", :html="true", :toc="true", tocId="data-set-toc").
        ...
        2. 發行日期： 2023/08/31
        3. 預計失效日期： 2023/12/30
           1. 0.3 為過度版本，僅使用在季度、年度資料，預計從 2024 開始，會統一所有資料集格式
        4. 資料集更動：
           1. 原始資料集無任何更動
           2. 由於消除重複的處理工具調整，消除重複資料集中的欄位，將有以下調整：
              1. 「最大刊登者編碼」改為「常見刊登者編碼」
              2. 「約略地點範圍」改為兩個欄位：「常見約略地點_x」、「常見約略地點_y」
              3. 新增「物件最後更新時間」
           3. 詳細解釋，請見〈[消除重複住宅邏輯更新](/blog/post/clickhouse-local-aggregation/)〉
        ...
        1. 預計在下一版新增物件標題資訊，但不會包含地址與說明

        ### 可能無法抓到的資料

        1. 如果物件在短時間內太過頻繁更新，則本資料集只會收錄其最後的狀態
        1. 在消除重複的資料集中，若為多筆物件合併，則會收錄其出現頻率最高的狀態
