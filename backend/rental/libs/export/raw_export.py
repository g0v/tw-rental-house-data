...
from rental.libs.export import Export
from rental.models import House, HouseEtc
from rental import enums

class RawExport(metaclass=Export):
    def init(self):
        headers = [
            Field('vendor_house_id', '物件編號'),
            Field('vendor', '租屋平台', field='name'),
            ...
            Field('apt_feature_code', '格局編碼（陽台/衛浴/房/廳）',
                fn=lambda x: '_Field()'.format(x) if x else ''),
            ...
            Field('living_functions', field='school', '附近有_學校？'),
            Field('living_functions', field='park', '附近有_公園？'),
            Field('living_functions', field='dept_store', '附近有_百貨公司？'),
            Field('living_functions', field='conv_store', '附近有_超商？'),
            Field('living_functions', field='traditional_mkt', '附近有_傳統市場？'),
            Field('living_functions', field='night_mkt', '附近有_夜市？'),
            Field('living_functions', field='hospital', '附近有_醫療機構？'),
            # Field('detail_dict', '附近交通_原始', field='transportation',
            #     fn=lambda x: '/'.join(x['environment'].get('附近交通', []))),
            Field('transportation', field='subway', '附近的捷運站數'),
            Field('transportation', field='bus', '附近的公車站數'),
            Field('transportation', field='train', '附近的火車站數'),
            Field('transportation', field='hsr', '附近的高鐵站數'),
            Field('transportation', field='public_bike', '附近的公共自行車數（實驗中）'),
            ...
            Field('author', '刊登者編碼', fn=lambda x: str(x.uuid) if hasattr(x, 'uuid') else None),
